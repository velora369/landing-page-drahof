...
// Componente para pré-carregar as imagens - versão simplificada
...
const ImagePreloader: React.FC<ImageLoaderProps> = ({ beforeImg, afterImg, onImagesLoaded }) => {
  useEffect(() => {
    ...
    // Handler para quando ambas as imagens estiverem carregadas
    const checkBothLoaded = () => {
      if (beforeLoaded && afterLoaded) {
        // Simplesmente notificar que as imagens estão prontas
        onImagesLoaded();
      }
    };
    ...
    // Limpar se o componente desmontar
    return () => {
      beforeImage.onload = null;
      afterImage.onload = null;
    };
  }, [beforeImg, afterImg, onImagesLoaded]);

  return null; // Este componente não renderiza nada visível
};

// Componente do slider antes e depois aprimorado
...
const BeforeAfterSlider: React.FC<BeforeAfterSliderProps> = ({ beforeImg, afterImg, title, onReady }) => {
  ...
  // Função simplificada para iniciar o slider (sem processamento canvas que causa erros de CORS)
  const handleImagesLoaded = () => {
    // Definir dimensões padrão para o slider
    const targetWidth = 1200;
    const targetHeight = 900;
    
    // Simplesmente definir que as imagens estão prontas, usando as URLs originais
    setProcessedImages({
      beforeUrl: beforeImg,
      afterUrl: afterImg,
      width: targetWidth,
    ...
    // Sinalizar que o carregamento está completo
    setIsLoading(false);
    if (onReady) onReady();
  };
  ...
  return (
    <>
      {/* Componente de pré-carregamento que processa as imagens */}
      <ImagePreloader
        beforeImg={beforeImg}
        afterImg={afterImg}
      ...
      {isLoading && (
        <div className="w-full h-[450px] bg-gray-100 rounded-2xl flex items-center justify-center">
          <div className="flex flex-col items-center text-gray-600">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[#731C13] mb-4"></div>
            <p>Processando imagens...</p>
            <p className="text-sm text-gray-500 mt-2">Garantindo o alinhamento perfeito</p>
          </div>
        </div>
      )}
      
      {/* Slider real com imagens processadas */}
      {!isLoading && processedImages && (
        <motion.div 
          ...
          style={{
            ...
            aspectRatio: `${processedImages.width}/${processedImages.height}`,
          }}
        >
          {/* Imagem do resultado (depois) */}
          <img 
            src={processedImages.afterUrl}
            alt="Depois"
          ...
          <div 
            className="absolute inset-y-0 w-1 bg-white shadow-md cursor-col-resize"
          ...
          >
            {/* Alça do slider */}
            <div 
              className="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 w-12 h-12 bg-white rounded-full shadow-lg flex items-center justify-center cursor-grab active:cursor-grabbing z-10 border-2 border-[#731C13] group-hover:scale-110 transition-transform duration-300"
              style={{ left: '50%' }}
            >
          ...
          <div className="absolute top-4 left-4 bg-[#425F70]/80 backdrop-blur-sm text-white px-3 py-1 rounded-md text-sm font-medium shadow-md">
            Antes
          </div>
          <div className="absolute top-4 right-4 bg-[#731C13]/80 backdrop-blur-sm text-white px-3 py-1 rounded-md text-sm font-medium shadow-md">
            Depois
          </div>
          
          {/* Título do caso */}
          {title && (
            <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/70 via-black/40 to-transparent pt-12 pb-4 px-6">
...
const ThumbnailCard: React.FC<ThumbnailCardProps> = ({ caseItem, onClick, index }) => {
  return (
    ...
    >
      <div className="h-48 bg-cover bg-center relative overflow-hidden" style={{ backgroundImage: `url(${caseItem.thumbnail || caseItem.after})` }}>
        {/* Gradiente overlay */}
...
export default function BeforeAfterEnhanced() {
  ...
  // Handler quando o slider estiver pronto
  ...
  return (
    ...
    >
      ...
      <div className="container mx-auto max-w-6xl relative z-10">
        ...
        {/* Slider principal interativo com as imagens redimensionadas */}
      ...
      {modalOpen && (
        ...
        >
          ...
          >
            ...
            {!sliderReady && (
              <div className="flex justify-center items-center p-8" style={{ height: '450px' }}>
                <div className="flex flex-col items-center">
                  <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[#731C13]"></div>
                  <p className="mt-4 text-gray-600">Otimizando imagens para comparação...</p>
                </div>
              </div>
            )}
